...
export type SubscriberImportPayload = { rows: SubscriberImportRow[]; listIds: string[] };
...
type Props = {
  ...
  onImport?: (payload: SubscriberImportPayload) => Promise<{ imported: number; failed: number }>;
};

/** Serialize an ImportTarget for a <select> value. */
const targetValue = (t: ImportTarget): string =>
  t.kind === 'attribute' ? `attr:${t.key}` : t.kind;
const parseTargetValue = (v: string): ImportTarget =>
  v.startsWith('attr:') ? { kind: 'attribute', key: v.slice(5) } : ({ kind: v } as ImportTarget);
...
export default function SubscriberEditorModal({
  ...
  const runImport = async () => {
    ...
    try {
      const res = await onImport({ rows: built.rows, listIds: importListIds });
      setImportResult(res);
      setImportPhase('done');
    } catch {
        ...
        {importPhase === 'done' && importResult ? (
          <div className={styles.result}>
            ...
            <p className={styles.resultTitle}>
              {importResult.imported.toLocaleString('en-US')}{' '}
              {importResult.imported === 1 ? 'subscriber' : 'subscribers'} imported
            </p>
