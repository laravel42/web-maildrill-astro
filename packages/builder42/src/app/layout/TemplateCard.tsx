/**
 * Tarjetas de plantilla (página/sección) — extraído de `TemplatesPanel.tsx`
 * para reutilizarse también desde `Sidebar.tsx` (docs/builder-sections-plan.md
 * — ronda de reorganización: la categoría "section" pasa del panel
 * "Plantillas" al panel "Componentes", como último grupo de la paleta).
 *
 * Miniatura visual (docs/38 §2.2), preview cacheado por `layoutId`
 * (`previewCache`, module-level: sobrevive mientras el editor esté montado,
 * independientemente de qué panel la solicite) y las dos tarjetas
 * (`SectionTemplateCard` para pick & insert vía drag/tap,
 * `PageTemplateCard` para reemplazo de página con confirmación).
 */
 ...
 * mientras el editor esté montado (docs/38 §2.3) — el html/css del fragmento no
...
/** Tarjeta de plantilla de sección: pick & insert vía drag (o tap en touch). */
export function SectionTemplateCard({ layout }: { layout: SectionLayoutDefinition }) {
  const { t } = useTranslation("sidebar");
...
/** Tarjeta de plantilla de página: reemplazo con confirmación (modal aparte). */
