...
function Field({ label, hint, error, counter, htmlFor, className, children }: FieldProps) {
    ...
    <div data-slot="field" className={cn('flex flex-col gap-1.5', className)}>
      {hasHeader && (
        <div className="flex items-center justify-between gap-2">
          {label != null ? <Label htmlFor={htmlFor}>{label}</Label> : <span aria-hidden="true" />}
          {counter != null && (
            <span data-slot="field-counter" className="text-xs tabular-nums text-muted-foreground">
              {counter}
            </span>
          )}
        </div>
