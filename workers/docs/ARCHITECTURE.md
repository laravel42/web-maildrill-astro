...
> **Current ops truth:** [`../HANDOFF.md`](../HANDOFF.md) and
> [`posthog-infobip-hog.md`](./posthog-infobip-hog.md). Production DLRs go
> **Infobip → PostHog Hog**; product message/campaign completion is the
> **campaign-delivery** HogQL poller. Sections below that treat Infobip→Maildrill
> webhooks as the primary DLR path are **historical** (routes remain for mock/tests).

## What this is

The asynchronous messaging engine for Maildrill: a headless **Fastify (Node) service + BullMQ workers** that turn durable PostgreSQL records into provider sends (Infobip first), ingest delivery/engagement webhooks (and, in production, reconcile DLRs from PostHog), and enforce retries, rate limits, idempotency, and multitenant isolation.

This is the **messaging engine** (`apps/api`) — one app in the backend monorepo. The product data & delivery model (subscribers, lists, segments, templates) is now implemented alongside it in `apps/product-api` + `packages/product`; its schema is in [infobip-api-scheme.md](./infobip-api-scheme.md). What stays out of the backend entirely: the **visual editors** — template designer, drag-and-drop campaign builder — which live in the frontend. §26 lists this engine's own non-goals; the README is canonical for the full, current service.
