...
export async function authRoutes(appRaw: FastifyInstance): Promise<void> {
  ...
  app.post(
    ...
    { schema: { tags: TAG, summary: "Email a 6-digit sign-in code (+ auto-login link)", body: requestSchema } },
    async (req, reply) => {
  ...
  app.post(
    "/v1/auth/code/verify",
    { schema: { tags: TAG, summary: "Verify a login code, returning identity + workspaces", body: verifySchema } },
    async (req, reply) => {
