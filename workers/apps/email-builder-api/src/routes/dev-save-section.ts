...
export const devSaveSectionPlugin = async function devSaveSectionPlugin(fastify: FastifyInstance) {
  ...
  fastify.post('/dev/save-section', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (thumbnail) {
      try {
        writeThumbnailFile(getThumbnailPath('sections', body.role, id, thumbnail.format), thumbnail.bytes);
      } catch (err) {
    ...
    try {
      ...
      if (thumbnail) deleteThumbnailFile(getThumbnailPath('sections', body.role, id, thumbnail.format));
  ...
  fastify.get<{ Params: { role: string; id: string } }>('/dev/sections/:role/:id', async (request, reply) => {
    if (!isLibraryEndpointEnabled()) return reply.status(403).send(DISABLED_RESPONSE_BODY);
    const { role, id } = request.params;
    if (!ROLE_VALUES.includes(role as Role)) return reply.status(400).send({ error: 'invalid_role', role });
  ...
  fastify.put<{ Params: { role: string; id: string } }>('/dev/sections/:role/:id', async (request, reply) => {
    if (!isLibraryEndpointEnabled()) return reply.status(403).send(DISABLED_RESPONSE_BODY);
    const { role, id } = request.params;
    if (!ROLE_VALUES.includes(role as Role)) return reply.status(400).send({ error: 'invalid_role', role });
    if (!isValidUuid(id)) return reply.status(400).send({ error: 'invalid_id', id });
    ...
    if (thumbnail) {
      try {
        writeThumbnailFile(getThumbnailPath('sections', role as Role, id, thumbnail.format), thumbnail.bytes);
      } catch (err) {
        if (err instanceof ThumbnailTooLargeError)
          return reply
            .status(413)
      ...
      deleteThumbnailFile(getThumbnailPath('sections', role as Role, id, thumbnail.format === 'webp' ? 'png' : 'webp'));
    }
  ...
  fastify.delete<{ Params: { role: string; id: string } }>('/dev/sections/:role/:id', async (request, reply) => {
    if (!isLibraryEndpointEnabled()) return reply.status(403).send(DISABLED_RESPONSE_BODY);
    const { role, id } = request.params;
    if (!ROLE_VALUES.includes(role as Role)) return reply.status(400).send({ error: 'invalid_role', role });
    if (!isValidUuid(id)) return reply.status(400).send({ error: 'invalid_id', id });
  ...
  fastify.get<{ Params: { role: string; id: string } }>('/dev/sections/:role/:id/thumbnail', async (request, reply) => {
    if (!isLibraryEndpointEnabled()) return reply.status(403).send(DISABLED_RESPONSE_BODY);
    const { role, id } = request.params;
    if (!ROLE_VALUES.includes(role as Role)) return reply.status(400).send({ error: 'invalid_role', role });
    if (!isValidUuid(id)) return reply.status(400).send({ error: 'invalid_id', id });
    ...
    const candidates = [
      { filePath: getThumbnailPath('sections', role as Role, id, 'webp'), contentType: 'image/webp' },
      { filePath: getThumbnailPath('sections', role as Role, id, 'png'), contentType: 'image/png' },
    ];
    ...
    if (chosen === null) return reply.status(404).send({ error: 'thumbnail_not_found', role, id });
    ...
    if (buffer === null) return reply.status(404).send({ error: 'thumbnail_not_found', role, id });
