...
const PLACEHOLDER_PATTERNS: { pattern: RegExp; label: string }[] = [
  ...
  {
    pattern: /\byour (?:company|brand|business) name\b/i,
    label: 'a "Your Company Name" placeholder',
  },
...
export function structureRules(ctx: RuleContext): Finding[] {
  ...
  if (doc.root.type !== 'EmailLayout') {
    out.push(
      finding({
        ...
        impact:
          'The renderer expects EmailLayout at the root to produce the canvas; anything else renders unstyled or not at all.',
  ...
  if (doc.danglingRefs.length > 0) {
    ...
    out.push(
      finding({
        ...
        impact:
          'The renderer skips the missing children, so the email silently loses content between preview and send.',
  ...
  if (doc.duplicateRefs.length > 0) {
    out.push(
      finding({
        ...
        impact:
          'Only the first occurrence renders, and editing the block changes it in every place it appears.',
  ...
  if (doc.orphanIds.length > 0) {
    out.push(
      finding({
        ...
        impact:
          'The content was authored but will not render — usually a generation or drag-and-drop bug rather than an intentional draft.',
    ...
    out.push(
      finding({
        ...
        detail: placeholderHits
          ...
          .join('; '),
        impact:
          'Placeholder copy reaching an inbox reads as a broken send and costs more trust than the email earns.',
  ...
  if (emptyText.length > 0) {
    out.push(
      finding({
        ...
        impact:
          'They still contribute padding, so they open unexplained gaps in the vertical rhythm.',
  ...
  for (const block of doc.blocks.filter((b) => b.type === 'ColumnsContainer')) {
    ...
    const populated = columns
      .slice(0, declared)
      .filter(
        (c) => Array.isArray(c?.childrenIds) && (c.childrenIds as unknown[]).length > 0,
      ).length;
    if (populated < declared) {
      out.push(
        finding({
          ...
          impact:
            'Empty columns still consume horizontal space, pushing the real content into a narrower area than intended.',
  ...
  if (facts.metrics.blockCount < THRESHOLDS.minBlockCount) {
    out.push(
      finding({
        ...
        impact:
          'The email is likely missing structure a recipient expects — a header, a closing, or a footer.',
  ...
  if (facts.metrics.blockCount > THRESHOLDS.maxBlockCount) {
    out.push(
      finding({
        ...
        impact:
          'Long emails get clipped by Gmail, and the extra content dilutes the single action the email should drive.',
  ...
  if (facts.metrics.maxNestingDepth > THRESHOLDS.maxNestingDepth) {
    ...
    out.push(
      finding({
        ...
        impact:
          'Each level adds a wrapper table; deep nesting compounds rounding differences between clients and makes the layout fragile.',
