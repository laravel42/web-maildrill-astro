...
export const SUPPORT: Record<string, SupportFact> = {
  ...
  overflowHidden: {
    ...
    failsIn: ['outlookWindows'],
    effect: 'Backgrounds are not clipped to rounded corners, so colour bleeds past the rounded edge.',
    workaround: 'Keep the clipped area and its background the same colour so the bleed is invisible.',
  },
  objectFit: {
    feature: 'object-fit / object-position',
    failsIn: ['outlookWindows', 'gmailWeb', 'gmailApp3p', 'yahoo'],
    effect: 'The image is stretched to the declared box instead of being cropped, so it renders distorted.',
  ...
  webFonts: {
    ...
    effect: 'The font falls back to the next entry in the stack, changing text metrics and re-wrapping lines.',
    workaround:
  ...
  backgroundImage: {
    ...
    workaround: 'Add a VML fallback, or set a solid background-color that keeps foreground text readable on its own.',
  },
  displayInlineBlock: {
    feature: 'display: inline-block',
    failsIn: ['outlookWindows'],
    effect: 'The element is laid out as a block, so a shrink-to-fit pill stretches to the full container width.',
    workaround: 'Give the element an explicit width, or centre it with an align attribute on a wrapping table.',
  },
  maxWidth: {
    feature: 'max-width',
    failsIn: ['outlookWindows'],
  ...
  imagesOffByDefault: {
    ...
    effect: 'Images are blocked until the recipient opts in; only alt text and background colours are visible.',
    workaround: 'Give every image meaningful alt text and never let an image be the only carrier of a message or CTA.',
  },
};
...
export const FONT_FACTS: Record<string, FontFact> = {
  INHERIT: { key: 'INHERIT', label: 'Inherit', webFont: false, fallback: null, category: 'system', risk: 'none' },
  MODERN_SANS: { key: 'MODERN_SANS', label: 'Modern sans', webFont: false, fallback: 'Arial', category: 'system', risk: 'none' },
  WIDE_SANS: { key: 'WIDE_SANS', label: 'Wide sans', webFont: false, fallback: 'Verdana', category: 'system', risk: 'none' },
  COMPACT_SANS: { key: 'COMPACT_SANS', label: 'Compact sans', webFont: false, fallback: 'Tahoma', category: 'system', risk: 'none' },
  CLASSIC_SERIF: { key: 'CLASSIC_SERIF', label: 'Classic serif', webFont: false, fallback: 'Georgia', category: 'system', risk: 'none' },

  ROBOTO: { key: 'ROBOTO', label: 'Roboto', webFont: true, fallback: null, category: 'sans', risk: 'medium' },
  OPEN_SANS: { key: 'OPEN_SANS', label: 'Open Sans', webFont: true, fallback: null, category: 'sans', risk: 'medium' },
  LATO: { key: 'LATO', label: 'Lato', webFont: true, fallback: null, category: 'sans', risk: 'medium' },
  MONTSERRAT: { key: 'MONTSERRAT', label: 'Montserrat', webFont: true, fallback: null, category: 'sans', risk: 'medium' },
  POPPINS: { key: 'POPPINS', label: 'Poppins', webFont: true, fallback: 'Arial', category: 'sans', risk: 'low' },
  INTER: { key: 'INTER', label: 'Inter', webFont: true, fallback: 'Arial', category: 'sans', risk: 'low' },
  NUNITO: { key: 'NUNITO', label: 'Nunito', webFont: true, fallback: 'Arial', category: 'sans', risk: 'low' },
  RALEWAY: { key: 'RALEWAY', label: 'Raleway', webFont: true, fallback: 'Arial', category: 'sans', risk: 'low' },
  WORK_SANS: { key: 'WORK_SANS', label: 'Work Sans', webFont: true, fallback: 'Arial', category: 'sans', risk: 'low' },
  SOURCE_SANS: { key: 'SOURCE_SANS', label: 'Source Sans 3', webFont: true, fallback: 'Arial', category: 'sans', risk: 'low' },
  MULISH: { key: 'MULISH', label: 'Mulish', webFont: true, fallback: 'Arial', category: 'sans', risk: 'low' },
  RUBIK: { key: 'RUBIK', label: 'Rubik', webFont: true, fallback: 'Arial', category: 'sans', risk: 'low' },
  KARLA: { key: 'KARLA', label: 'Karla', webFont: true, fallback: 'Arial', category: 'sans', risk: 'low' },
  DM_SANS: { key: 'DM_SANS', label: 'DM Sans', webFont: true, fallback: 'Arial', category: 'sans', risk: 'low' },
  FIGTREE: { key: 'FIGTREE', label: 'Figtree', webFont: true, fallback: 'Arial', category: 'sans', risk: 'low' },
  PT_SANS: { key: 'PT_SANS', label: 'PT Sans', webFont: true, fallback: 'Arial', category: 'sans', risk: 'low' },
  OSWALD: { key: 'OSWALD', label: 'Oswald', webFont: true, fallback: null, category: 'display', risk: 'high' },

  MERRIWEATHER: { key: 'MERRIWEATHER', label: 'Merriweather', webFont: true, fallback: null, category: 'serif', risk: 'medium' },
  PLAYFAIR: { key: 'PLAYFAIR', label: 'Playfair', webFont: true, fallback: null, category: 'serif', risk: 'medium' },
  LORA: { key: 'LORA', label: 'Lora', webFont: true, fallback: 'Georgia', category: 'serif', risk: 'low' },
  PT_SERIF: { key: 'PT_SERIF', label: 'PT Serif', webFont: true, fallback: 'Georgia', category: 'serif', risk: 'low' },
  SOURCE_SERIF: { key: 'SOURCE_SERIF', label: 'Source Serif 4', webFont: true, fallback: 'Georgia', category: 'serif', risk: 'low' },
  ROBOTO_SLAB: { key: 'ROBOTO_SLAB', label: 'Roboto Slab', webFont: true, fallback: 'Georgia', category: 'serif', risk: 'low' },
  BITTER: { key: 'BITTER', label: 'Bitter', webFont: true, fallback: 'Georgia', category: 'serif', risk: 'low' },
  CRIMSON: { key: 'CRIMSON', label: 'Crimson Text', webFont: true, fallback: 'Georgia', category: 'serif', risk: 'low' },

  ROBOTO_MONO: { key: 'ROBOTO_MONO', label: 'Roboto Mono', webFont: true, fallback: 'Courier New', category: 'mono', risk: 'low' },
  JETBRAINS_MONO: { key: 'JETBRAINS_MONO', label: 'JetBrains Mono', webFont: true, fallback: 'Courier New', category: 'mono', risk: 'low' },

  PACIFICO: { key: 'PACIFICO', label: 'Pacifico', webFont: true, fallback: null, category: 'display', risk: 'high' },
  DANCING_SCRIPT: { key: 'DANCING_SCRIPT', label: 'Dancing Script', webFont: true, fallback: null, category: 'display', risk: 'high' },
  CAVEAT: { key: 'CAVEAT', label: 'Caveat', webFont: true, fallback: null, category: 'display', risk: 'high' },
};
